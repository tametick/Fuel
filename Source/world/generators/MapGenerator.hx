...
class MapGenerator {
	...
	function addPerlinTiles(width:Int, height:Int, baseMap:Array<Array<Int>>, tilesIndexes:Array<Int>, tilesWeights:Array<Int>, canHaveTile:Array<Int>, ?jaggedness:Int = 8) {
		...
		for (y in 0...height)
			for (x in 0...width)
				if ( Utils.exists(canHaveTile, baseMap[y][x]) && treeMap[y][x] != -1)
	...
	static function pointToNum(width:Int, height:Int, p:FlxPoint):Int {
		return Std.int((p.y*width)+p.x);
	}
	
	static function numToPoint(width:Int, height:Int, num:Int):FlxPoint {
		return new FlxPoint(num % width, Math.floor(num / width));
	}
	...
	static function isOpen(width:Int, height:Int, map:Array<Array<Int>>, openTiles:Array<Int>, x:Int, y:Int) {
		...
		if ( Utils.exists(openTiles, map[y][x]) ) return true;
		return false;
	}
	...
	static function isOpenArea(mapWidth:Int, mapHeight:Int, map:Array<Array<Int>>, openTiles:Array<Int>, x0:Int, y0:Int,x1:Int,y1:Int) {
		...
		for(yy in y0...y1)
			for (xx in x0...x1)
				if (!Utils.exists(openTiles, map[yy][xx]))
					return false;
		return true;
	}
	
	static function getValidStairsUpLocation(mapWidth:Int, mapHeight:Int, map:Array<Array<Int>>, wallTile, openTiles:Array<Int>):FlxPoint {
		for (y in 1...mapHeight-2)
			for (x in 1...mapWidth - 2) {
				if (map[y][x - 1] == wallTile && map[y][x] == wallTile && map[y][x + 1] == wallTile && Utils.exists(openTiles, map[y + 1][x]) )
					return new FlxPoint(x, y);
			}
		return null;
	}
	
	static function getValidStairsDownLocation(mapWidth:Int, mapHeight:Int, map:Array<Array<Int>>, wallTile, openTiles:Array<Int>):FlxPoint {
	...
	public static function isFullyConnected(width:Int, height:Int, map:Array<Array<Int>>, tilesToCount:Array<Int>, ?mapToMarkedDisconnectedAreas:Array<Array<Int>>=null, ?disconnectedMarker:Int=-1):Bool {
		if (mapToMarkedDisconnectedAreas != null) {
			...
			for (y in 0...height)
				for (x in 0...width) {
					if (Utils.exists(tilesToCount, map[y][x]))
						mapToMarkedDisconnectedAreas[y][x] = disconnectedMarker;
				}
		}
		
		var openCount = countTiles(width, height, map, tilesToCount);
		var openList = [];
		var closedList = [];
		var pass = false;
		
		// pick a random, valid starting location
		var start = getRandomTile(width, height, map, tilesToCount);
		...
		while ( openList.length > 0 ) {
			...
			var N = new FlxPoint(curPos.x, curPos.y - 1);
			var E = new FlxPoint(curPos.x+1, curPos.y);
			var S = new FlxPoint(curPos.x, curPos.y + 1);
			var W = new FlxPoint(curPos.x-1, curPos.y);
			...
			if ( isOpen(width,height,map,tilesToCount, Std.int(curPos.x), Std.int(curPos.y-1)) ) {
				...
				if ( !Utils.exists(openList, checkNum) && !Utils.exists(closedList, checkNum) ) {
	...
	public static function getRandomTile<T>(width:Int, height:Int, map:Array<Array<T>>, tileTypesToGet:Array<T>):FlxPoint {
		var x , y = 0;
		do {
			x = Utils.randomInt(width);
			y = Utils.randomInt(height);
		} while ( !Utils.exists(tileTypesToGet, map[y][x]) );
