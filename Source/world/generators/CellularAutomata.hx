...
class CellularAutomata {
	...
	public function getCaveMap():Array<Array<Int>> {
		...
		var markedMap = MapGenerator.cloneMap(caMap);
		var connected = MapGenerator.isFullyConnected(width, height, caMap, [floor], markedMap, -1);
		
		var accesibleTiles = MapGenerator.countTiles(width, height, markedMap, [floor]);
				
		if (accesibleTiles > width * height / 3) {
			MapGenerator.repalceAllTiles(width, height, markedMap, -1, wall);
			return markedMap;
		}
		else
			return getCaveMap();
